...
namespace DispatchData.Repositories
{
    public class ProductRepository
    {
        ...
        private IQueryable<DispatchCore.Models.Product> ApplyFilter(SearchOptions<DispatchCore.Models.Product> filter, IQueryable<DispatchCore.Models.Product> query)
        {
            if (filter != null)
            {
                if (filter.filter.VendorId != null)
                {
                    query = query.Where(s => s.VendorId == filter.filter.VendorId);
                }
            }
